...
export function MuiProjectsTable({ proyectos, resolutivos, municipios, giros, tiposEstudio, selectedDate }: MuiProjectsTableProps) {
  ...
  // Función para ordenar por fecha de ingreso más reciente
  ...
  const filteredProyectos = useMemo(() => {
    ...
    const filtered = proyectos.filter(proyecto => {
      // Manejar valores nulos de manera segura
  ...
  const filteredResolutivos = useMemo(() => {
    ...
    const filtered = resolutivos.filter(resolutivo => {
      ...
      // Filtros de fecha para resolutivos
      let matchesYear = true
      let matchesMonth = true
    ...
    <StyledCard elevation={0}>
      <CardContent sx={{ p: 4 }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          <Box>
            <Typography variant="h6" component="h3" fontWeight="semibold" color="text.primary">
              Tabla de proyectos ingresadosy resolutivos emitidos
            </Typography>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {/* Vista Desktop - Todos los filtros en una fila */}
            <Box sx={{ 
              ...
              <FormControl size="small" sx={{ minWidth: 120 }}>
                ...
                >
                  <MenuItem value="all">Todos</MenuItem>
                  {giros.map(giro => (
                    <MenuItem key={giro} value={giro}>{giro}</MenuItem>
                  ))}
                </Select>
              </FormControl>
              <FormControl size="small" sx={{ minWidth: 120 }}>
            ...
            {/* Vista Mobile - Solo búsqueda y botones */}
            <Box sx={{ 
                      ...
                      paginatedProyectos.map((proyecto) => (
                        <TableRow 
                          ...
                          sx={{ 
                            ...
                            '&:hover': {
                              backgroundColor: proyecto.coordenadas_x && proyecto.coordenadas_y ? 'rgba(0, 0, 0, 0.04)' : 'inherit'
                            }
                          }}
                        >
                      ...
                      paginatedResolutivos.map((resolutivo) => (
                        <TableRow 
                          ...
                          sx={{ 
                            ...
                            '&:hover': {
                              backgroundColor: resolutivo.coordenadas_x && resolutivo.coordenadas_y ? 'rgba(0, 0, 0, 0.04)' : 'inherit'
                            }
                          }}
                        >
