...
async function generateStaticPages() {
  try {
    ...
    // Generar página HTML para cada boletín
    for (const boletin of recentBoletines) {
      const boletinDir = path.join(staticDir, boletin.id.toString())
      if (!fs.existsSync(boletinDir)) {
        fs.mkdirSync(boletinDir, { recursive: true })
      }
      
      const htmlContent = generateBoletinHTML(boletin)
      const indexPath = path.join(boletinDir, 'index.html')
      
      fs.writeFileSync(indexPath, htmlContent)
      console.log(`Generado: ${indexPath}`)
    }
...
function generateBoletinHTML(boletin) {
...
<head>
    ...
    <title>Resumen Boletín SSMAA #${boletin.id} | ADN-Aguascalientes</title>
    <meta name="description" content="Resumen detallado del Boletín Ambiental de la SSMAA #${boletin.id}. Proyectos ingresados, resolutivos emitidos y mapas de ubicación.">
    
    <!-- OpenGraph -->
    <meta property="og:title" content="Resumen Boletín SSMAA #${boletin.id} | ADN-Aguascalientes">
    <meta property="og:description" content="Resumen detallado del Boletín Ambiental de la SSMAA #${boletin.id}. Proyectos ingresados, resolutivos emitidos y mapas de ubicación.">
    <meta property="og:url" content="https://adn-a.org/boletines-ssmaa/resumen/${boletin.id}">
    <meta property="og:image" content="https://adn-a.org/assets/miniaturaboletines.png">
    <meta property="og:type" content="website">
    
    <!-- Twitter -->
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="Resumen Boletín SSMAA #${boletin.id}">
    <meta name="twitter:description" content="Resumen detallado del Boletín Ambiental de la SSMAA #${boletin.id}. Proyectos ingresados, resolutivos emitidos y mapas de ubicación.">
...
<body>
    <div class="container">
        <div class="header">
            ...
            <h2>${formatearFechaCorta(boletin.fecha_publicacion)}</h2>
        </div>
        ...
        <div class="content">
                    ...
                    <div class="project-card">
                        ...
                        <div class="project-info">
                            <div class="project-details">
                                ...
                                <div class="info-item">
                                    <span class="info-label">Fechas de ingreso:</span>
            ...
            ${boletin.resolutivos_emitidos && boletin.resolutivos_emitidos.length > 0 ? `
                ${boletin.resolutivos_emitidos.map((resolutivo, index) => `
                    <div class="project-card">
                        <h3 class="project-title">${resolutivo.nombre_proyecto}</h3>
                        <div class="project-info">
                            <div class="project-details">
                                <div class="info-item">
                                    <span class="info-label">Promovente:</span>
                                    <span class="info-value">${resolutivo.promovente}</span>
                                </div>
                                <div class="info-item">
                                    <span class="info-label">Municipio:</span>
                                    <span class="info-value">${resolutivo.municipio}</span>
                                </div>
                                <div class="info-item">
                                    <span class="info-label">Expediente:</span>
                                    <span class="info-value">${resolutivo.expediente}</span>
                                </div>
                                <div class="info-item">
                                    <span class="info-label">Fecha de ingreso:</span>
                                    <span class="info-value">${formatearFechaCorta(resolutivo.fecha_ingreso)}</span>
                                </div>
                                <div class="info-item">
                                    <span class="info-label">Fecha de resolutivo:</span>
                                    <span class="info-value">${formatearFechaCorta(resolutivo.fecha_resolutivo)}</span>
                                </div>
                                <div class="info-item">
                                    <span class="info-label">Tipo:</span>
                                    <span class="info-value">${resolutivo.tipo_estudio}</span>
                                </div>
                                <div class="info-item">
                                    <span class="info-label">Naturaleza del proyecto:</span>
                                    <div class="naturaleza">${resolutivo.naturaleza_proyecto}</div>
                                </div>
                            </div>
                            <div class="map-container">
                                ${resolutivo.coordenadas_x && resolutivo.coordenadas_y ? 
                                    `<img src="https://staticmap.openstreetmap.de/staticmap.php?center=${resolutivo.coordenadas_y},${resolutivo.coordenadas_x}&zoom=15&size=400x300&markers=${resolutivo.coordenadas_y},${resolutivo.coordenadas_x},red" alt="Mapa de ubicación" class="map-image">` :
                                    '<div style="display: flex; align-items: center; justify-content: center; height: 100%; background: #f5f5f5; color: #6B7280;">Sin coordenadas</div>'
                                }
                                <div class="map-info">
                                    Ubicación en ${resolutivo.municipio}
                                    ${resolutivo.coordenadas_x && resolutivo.coordenadas_y ? 
                                        `<br><a href="https://www.openstreetmap.org/?mlat=${resolutivo.coordenadas_y}&mlon=${resolutivo.coordenadas_x}&zoom=15" target="_blank" class="map-link">Ver en OpenStreetMap</a>` : ''
                                    }
                                </div>
                            </div>
                        </div>
